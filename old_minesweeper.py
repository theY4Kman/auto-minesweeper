...
from plotdevice.gfx.colors import Color
...
class Templates(object):
    ...
    def __init__(self):
        ...
        self._template_fns = {
            ...
            'depressed': self._draw_depressed,
            'empty': lambda ctx: None,
            'flag': self._draw_flag,
            'flag_wrong': self._draw_flag_wrong,
        }
        self._templates = {}
    ...
    def _draw_flag_wrong(self, ctx):
        self._draw_flag(ctx)

        margin = 2
        ctx.strokewidth(2)
        with ctx.stroke('red', 0.8), ctx.bezier():
            ctx.moveto(margin, margin)
            ctx.lineto(CELL_PX - margin, CELL_PX - margin)

            ctx.moveto(CELL_PX - margin, margin)
            ctx.lineto(margin, CELL_PX - margin)

    def _draw_unrevealed(self, ctx):
        self._draw_bevel(ctx, bg=Color('#ccc'),
                         top_left=Color('white', 0.6),
                         bottom_right=Color('black', 0.4))

    def _draw_depressed(self, ctx):
        self._draw_bevel(ctx, bg=Color('#aaa'),
                         top_left=Color('white', 0.5),
                         bottom_right=Color('white', 0),
                         border=2)

    def _draw_bevel(self, ctx, bg, top_left, bottom_right, border=BORDER_PX):
        """
        Draw a bezeled border, given its colors and width
        """
        border_px_0 = border - 1
        ...
        with ctx.nostroke():
            # Left, top border
            with ctx.fill(top_left), ctx.bezier():
                ctx.moveto(0, 0)
            ...
            # Right, bottom border
            with ctx.fill(bottom_right), ctx.bezier():
